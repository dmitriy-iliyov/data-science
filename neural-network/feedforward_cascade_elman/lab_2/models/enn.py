...
class ENN(Network, Trackable):
    ...
    def __init__(self, input_neurons=2, hidden_layer_count=1, hidden_neurons=15):
        ...
        for i in range(hidden_layer_count):
            ...
            self._context_hidden_w_list.append(
                tf.Variable(tf.random.uniform([previous_neurons_count, hidden_neurons], -1, 1), dtype=tf.float32))
            self._context_hidden_output_list.append(
                tf.zeros((previous_neurons_count, hidden_neurons), dtype=tf.float32))
        self._output_w = tf.Variable(tf.random.uniform([hidden_neurons, 1], -1, 1),
                                     dtype=tf.float32)
        ...
        self.checkpoint_dir = "checkpoints/enn_model/"
        self.checkpoint_manager = tf.train.CheckpointManager(self.checkpoint, self.checkpoint_dir, max_to_keep=3)

    def predict(self, batch):
        outputs = []
        self._context_hidden_output_list = [tf.zeros_like(b) for b in self._hidden_b_list]
        for t in range(batch.shape[1]):
            hidden_layer_output = batch[:, t, :]
            for i in range(self._hidden_layer_count):
                hidden_layer_output = self._activation_relu(
                    tf.matmul(hidden_layer_output, self._hidden_w_list[i]) +
                    tf.matmul(tf.expand_dims(self._context_hidden_output_list[i], 0), self._context_hidden_w_list[i]) +
                    self._hidden_b_list[i])
                self._context_hidden_output_list[i] = hidden_layer_output
            output = tf.matmul(hidden_layer_output, self._output_w) + self._output_b
            outputs.append(output.numpy())
        return np.array([output[0] for output in outputs[0]])
    ...
    def fit(self, train_data, train_answers, epochs=1000, learning_rate=0.05, batch_size=1):
        ...
        for epoch in range(1, epochs + 1):
            ...
            for i in range(0, len(train_data), batch_size):
                ...
                with tf.GradientTape() as tape:
                    outputs = []
                    for t in range(batch_data.shape[1]):
                        current_input = batch_data[:, t, :]
                        output = self._fit_forward(current_input)
                        outputs.append(output)
                    # self._debug(outputs[-1], batch_answers)
                    mse = self._compute_mse(outputs[-1], batch_answers)
                ...
                gradients = tape.gradient(mse,
                                          [*self._hidden_w_list, *self._hidden_b_list, self._output_w, self._output_b])
                optimizer.apply_gradients(
                    zip(gradients, [*self._hidden_w_list, *self._hidden_b_list, self._output_w, self._output_b]))
        ...
        filer.save_json(
            '/Users/sayner/github_repos/neural-network/feedforward_cascade_elman/lab_2/data_files/statistics/enn_statistic.txt',
            statistic)
        return statistic
    ...
    def save_model(self):
        save_path = self.checkpoint.save(file_prefix=self.checkpoint_dir + 'enn_model')
    ...
    def _debug(self, output, batch_answers):
        for i in range(len(batch_answers)):
            print(f"{output[i]} : {batch_answers[i]}")
